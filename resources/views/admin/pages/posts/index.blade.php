            ...
            <div class="x_content" class="list-table">
              <table class="table table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    <td><img style="width:100px;height:100px" src="{{ $post['user']['userInfo']['avatar_url'] }}" alt=""></td>
                    ...
                    <td>
                      <form action="" class="col-md-4" method="POST">
                        @method('PUT')
                        @csrf
                        <button id="update{{ $post['id'] }}" onclick="updateStatus(event, {{ $post['id'] }}, '{{ route('admin.api.posts.update.status', ['id' => $post['id']]) }}')" class="btn btn-primary update-btn" type="button">
                          @if ($post['status'])
                      ...
                      <form action="{{ route('admin.posts.destroy', ['id' => $post['id']]) }}" class="col-md-4" method="POST" id="deleted{{ $post['id'] }}">
                        @csrf
                        @method('DELETE')
                        <button onclick="deleteRecord(event, {{ $post['id'] }})" class="btn btn-danger" type="submit"><i class="fa fa-trash icon-size" ></i></button>
                      </form>
